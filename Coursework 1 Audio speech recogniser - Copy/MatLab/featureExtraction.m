function [magSpec] = featureExtraction(speechFile, ibmnoiseFile, ifNoisy, numChannels, numFilters, path, fileOutputName)

%Read in audio data
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[speechData, fs] = audioread(speechFile);
if ibmnoiseFile ~= 0
    [noiseData, fs] = audioread(ibmnoiseFile);
    noiseData = noiseData * find_alpha(speechData,noiseData,10);
end
if fs ~= 16000
...
for frame = 1:numFrames*2 - 1 %Chop off last frame as overlapping would go beyond array
    ...
    energyArray(frame) = signal_power(shortTimeFrame);
